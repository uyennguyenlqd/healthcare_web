...
// Define the interface for the file object
interface File {
  file_id: string;
  filename: string;
  status: string;
...
const FileViewer = () => {
  ...
  const [files, setFiles] = useState<File[]>([]); // Correctly type the state
  ...
  const handleFileUpload = async (
    ...
    const files = event.target.files; // files will be of type FileList | null
    if (files && files.length > 0) {
      const data = new FormData();
      data.append("file", files[0]); // Append the first selected file
      await fetch("/api/assistants/files", {
        method: "POST",
        body: data,
      });
